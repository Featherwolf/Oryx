...
| `rcpc` | `LDAPR` / `STLR` | **0** | **> 0** | exact on W→R (the cheap win) — MCA/IRIW pending |
| `sc` | `LDAR` / `STLR` | **0** | **0** | correct but **over-strong** (also kills W→R) |
| `dmb` | `LDR;DMB ISHLD` / `DMB ISHST;STR` | **0** | **> 0** | exact-TSO via fences, MCA-safe (pre-8.3 fallback) |
...
2. **W→R exactness (half of it).** SB-allowed must be **> 0** for `rcpc` and `dmb` but **0** for
   `sc`. Seeing SB under `rcpc` while `sc` suppresses it proves `LDAPR`/`STLR` matches TSO on the
   store→load axis — it keeps the store-buffer (W→R) outcome TSO permits, so it costs nothing
   extra, while `sc` is provably over-strong. The script prints `W->R EXACTNESS CONFIRMED`.

The script auto-scans core pairs until the `plain` SB control fires (so you rarely need to tune),
then prints `LAYER A: ordering restored by all three mappings` plus the W→R exactness line. If
