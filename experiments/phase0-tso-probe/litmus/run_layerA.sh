...
if [ "$fail" -eq 0 ]; then
    ...
    echo ""
    echo " ON rcpc CLEARANCE: the SB result above IS the complete rcpc-vs-sc test."
    echo "   The ONLY thing RCpc relaxes vs RCsc is the store->load (SB) edge; WRC and"
    echo "   IRIW contain no such edge, so rcpc and sc forbid them IDENTICALLY (both via"
    echo "   ARMv8 hardware multi-copy-atomicity). So W->R EXACTNESS CONFIRMED here"
    echo "   clears rcpc as exact-TSO — there is no separate 'MCA gate' rcpc must pass"
    echo "   that sc doesn't. (Optional: 'make mca' is a hardware-MCA sanity check that"
    echo "   confirms WRC stays forbidden under every ordered mapping on this silicon.)"
else
