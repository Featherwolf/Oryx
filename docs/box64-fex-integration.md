...
must preserve R→R, R→W, W→W, may relax W→R, **and preserve multi-copy atomicity**. That last
clause is the correction:

| x86 | ✅ Minimal exact-TSO (Arancini-proven) | Correct-but-stronger | ❌ Do NOT use |
|-----|----------------------------------------|----------------------|--------------|
| load | `LDR; DMB ISHLD` (trailing read fence) | `LDAR` (RCsc → SC) | **`LDAPR` alone** |
| store | `DMB ISHST; STR` (leading write fence) | `STLR` | — |
| MFENCE | `DMB ISH` | `DMB ISH` | — |
| LOCK RMW / XCHG / CMPXCHG | LSE **`-AL`** atomic (`CASAL`/`SWPAL`/`LDADDAL`) — full acq+rel | ll/sc `LDAXR/STLXR` loop | acquire-only / bare CAS |

**Why not bare `LDAPR`:** RCpc is *weaker* than TSO — it relaxes multi-copy atomicity, so `LDAPR`
can read a release value not yet globally observed, permitting **WRC-style causality violations
x86 forbids**. **[V!]** `LDAR` (RCsc) is *stronger* than TSO (forbids the W→R reorder TSO allows)
— correct but over-costs; it's the right **conservative baseline** for differential testing and is
what our reference `liboryxtu` currently emits (safe). The **minimal exact-TSO** choice on ARM
(other-multi-copy-atomic since ARMv8) is the **DMB-fence scheme**, which also enables cross-access
fence elimination/merging (Risotto/Arancini).
...
1. **On-device litmus battery — userspace `litmus7`** (herdtools) runs on Android with **no root,
   ...
   that kills the bare-LDAPR mapping — run them to catch exactly that class of bug. Our Phase-0
   MP/SB harness is the 2-thread seed; the 3–4-thread cases are best generated by `litmus7`.
