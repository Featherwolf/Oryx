...
> **`rcpc` ships only after its MCA half is measured on-device.** The claim above that `LDAPR`/`STLR`
> keeps WRC/IRIW forbidden rests on the *formal* ARMv8 model (other-MCA + `LDAPR` still ordering
> later accesses as an acquire), not yet on a measurement on Oryon. Our on-device Layer A test is
> 2-thread MP/SB — it proves the **W→R** half of exactness (`SB` fires under `LDAPR`/`STLR`, is
> suppressed under `LDAR`/`STLR`) but **cannot** see multi-copy atomicity. The MCA half needs the
> **3–4-thread WRC/IRIW** tests (best generated by `litmus7`), all of which must stay **forbidden**
> under `LDAPR`/`STLR`. Until `rcpc` passes WRC/IRIW on the actual silicon, the shipping default
> stays **`sc` (`LDAR`/`STLR`) or the DMB scheme**, both MCA-safe by construction — which is exactly
> why the reference translator defaults to `ORYX_ORDER_SC`. This is deliberate: a wrong `rcpc` bet
> is silent memory corruption, so the cheaper mapping is earned by a passing litmus, not assumed.
