...
- **Layer A mapping validator — runnable, no root** (`experiments/phase0-tso-probe/litmus/`, the
  ...
  install needed** for the 2-thread cases.
...
1. **Run Layer A on-device** (done tooling; `sh run_layerA.sh`): confirm ordering restored by all
   three mappings and the `rcpc`-fires / `sc`-suppresses `SB` exactness signature on Oryon. This
   is the first must-pass gate and needs no root. **Then extend to WRC/IRIW**: the 3–4-thread
   multi-copy-atomicity cases are best generated by `litmus7` (bundle it or its generated AArch64
   executables) rather than hand-rolled — all three mappings must keep WRC/IRIW forbidden.
2. **Prove exactness, both directions**: confirm all three mappings keep WRC/IRIW **forbidden**,
   *and* that the two exact-TSO mappings (`LDAPR`/`STLR`, DMB-fence) **exhibit `SB`** while SC does
   ...
   catching an under-strong one — Layer A's 2-thread SB check already does this half.
